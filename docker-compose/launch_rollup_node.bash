...
if [ "${ENV:-}" = "dev" ]; then
  exec rollup-node node --chain dev --datadir=/l2reth  --metrics=0.0.0.0:6060 --disable-discovery \
    ...
    --test \
    --consensus.algorithm noop --blob.mock \
  ...
  # Determine L1 URL
  if [ -n "${L1_URL}" ]; then
    # Use custom L1 URL if provided
    URL_PARAMS="--l1.url ${L1_URL}"
  elif [ "${SHADOW_FORK}" = "true" ]; then
    # Use shadow-fork L1 devnet
    wait_for_l1_devnet
    URL_PARAMS="--l1.url http://l1-devnet:8545"
  else
    # Use default Scroll Sepolia L1 RPC
    URL_PARAMS="--l1.url http://l1reth-rpc.sepolia.scroll.tech:8545"
  fi

  # Determine Blob S3 URL
  if [ -n "${BLOB_S3_URL}" ]; then
    BLOB_S3_PARAMS="--blob.s3_url ${BLOB_S3_URL}"
  else
  ...
  if [ -n "${L1_URL}" ]; then
    ...
    # Use default Scroll Mainnet L1 RPC
