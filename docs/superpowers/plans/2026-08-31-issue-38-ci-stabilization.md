...
```bash
cargo nextest run --all-features --locked --no-tests=pass -E 'kind(test) and not test(docker)' -- --skip test_should_consolidate_to_block_15k
```
...
```bash
cargo nextest run --all-features --locked --no-tests=pass -E 'test(docker_test_remote_block_source)' --test-threads=1
...
```bash
ALCHEMY_KEY= cargo nextest run --all-features --locked -E 'test(test_should_consolidate_to_block_15k)' --no-tests=pass
```
